...
def main():
    ...
    print("=" * 60)
    print()
    print("【云端】生成清单（XXH3，在云端项目根目录运行）:")
    print(f'  "{python}" "{gen}" .')
    print()
    print("【本机】dry-run 查看差异（下载 manifest.json.xz 后运行）:")
    print(f'  "{python}" "{build}" "{root_str}" "<manifest路径>" --dry-run')
    print()
    print("【本机】生成完全镜像同步包（默认，1g 分卷）:")
    print(f'  "{python}" "{build}" "{root_str}" "<manifest路径>"')
    print()
    print("【本机】生成增量更新包（只新增/更新，不删除/移动）:")
    print(f'  "{python}" "{build}" "{root_str}" "<manifest路径>" --mode incremental')
    print()
    print("【本机】生成完全镜像同步包（500m 分卷）:")
    print(f'  "{python}" "{build}" "{root_str}" "<manifest路径>" --volume-size 500m')
    print()
    print("【云端】解压同步包（在云端项目根目录运行）:")
    print(f'  7z x sync_<时间戳>.7z -o"{root_str}" -y')
    print()
    print("【云端】处理删除/移动列表（镜像模式如有，解压后进入 _apply_sync/ 双击 apply_sync.bat）:")
